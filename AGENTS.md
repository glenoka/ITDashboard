...
- **Telegram bot:** Notifikasi DOWN/RECOVERED (host/CCTV/UniFi/Ruijie) + perintah 2 arah via long polling `getUpdates` (tanpa webhook). Config di tabel `telegram_settings` (id=1), endpoint `GET/PUT /api/notifications/telegram/settings` + `POST .../test`. Perintah: `/cctv`, `/cctv <id|nama>` (kirim snapshot via `captureCameraSnapshot()`), `/procurement`, `/project`, `/project_add <judul>`, `/order_add`, `/ping <ip>`, `/status`, `/stats`, `/report`, `/backup`, `/restart`, `/cancel`, `/help`. Polling dimulai saat boot jika `enabled`, auto-restart saat settings diubah, stop jika token invalid. Hanya merespons `chat_id` terdaftar. **Inline keyboard** (`TG_KEYBOARD_MAIN` + `procurementKeyboard()`/`orderActionKeyboard()`/`cctvKeyboard()`) diproses di `handleTelegramCallback` (callback `callback_query`); tombol menjalankan handler yang sama seperti perintah `/`. Pesan `/start`/`/help` & `⚙️ Bantuan` di-`pin` via `telegramPinMessage` supaya menu selalu tampil di atas. **Tambah order** (`/order_add` atau tombol) = guided 2-langkah via state `telegramOrderState` (Map per `chat_id`: `waiting_pr` → `waiting_item`), timeout 10 menit, `/cancel` membatalkan; hasil `INSERT INTO it_orders` + broadcast `order_added`. **Project tick**: `/project` render tombol ✅ per item project (callback `prj:<id>`), logika toggle sama dengan endpoint REST, pesan di-update in-place via `telegramEditMessageText` (mencegah tombol basi). Checklist periode (harian/mingguan/bulanan/tahunan) **tidak** ditampilkan di Telegram. **Backup DB**: `runDailyBackup()` gzip `noc.db` → kirim `telegramSendDocument` ke chat terdaftar + simpan `backend/backups/noc-backup-YYYY-MM-DD.db.gz`, rotasi keep 14 (`BACKUP_KEEP`), jadwal 02:00. **Daily report**: `buildDailyReport()` (host/CCTV/order/project/sistem — tanpa checklist periode) via `/report` atau jadwal 07:00. Scheduler gabungan `startDailyJobs()` (setInterval 60s, flag `lastBackupDate`/`lastReportDate`). **Restart server** via `/restart`: `restartServer()` spawn proses baru (`node -e` dengan `setTimeout` 2.5s lalu `require(server.js)`, `detached` + `unref`) lalu `process.exit(0)` — cegah race EADDRINUSE; hanya dari chat_id terdaftar.
